  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        ...
        .json({
          message:
            "Erreur lors de la création ou de la modification du panier.",
        });
    }
  }
);
...
async function createOrder(req: Request, res: Response) {
  try {
    ...
    return res
      .status(201)
      .json({
        message: `Le produit ${product?.name} a été ajouté à votre panier.`,
      });
  } catch (error) {
    return res
      .status(500)
...
order.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const indexToRemove = existingOrder.products.findIndex(
      ...
      return res
        .status(200)
        .json({
          message: `Le produit ${product?.name} a été enlevé du panier.`,
        });
    } catch (error) {
      return res
        .status(500)
...
order.patch(
  "/:orderId/remove-products-with-same-id",
  authorize(["Client", "Gestionnaire", "Administrateur"]),
  async (req: Request, res: Response) => {
    try {
      const { orderId } = req.params;
      ...
      return res
        .status(200)
        .json({
          message: `Tous les ${product?.name} ont été enlevés du panier.`,
        });
    } catch (error) {
      return res
        .status(500)
